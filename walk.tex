...
and connected subgraphs $G_{i}:=(G_{i-1}-Q_{i})-D_{i}$, for
$i=1,\ldots,m+s$, where
...
and $G_i$ directly, as follows. As any induced subgraph of $2K_2$-free
graph is also $2K_2$-free, $G_i-Q_{i+1}$ is also a $2K_2$-free graph.
If $\omega(G_i)\ge3$ then by Lemma \ref{lm2}, there exists a
maximum dominating clique $Q_{i+1}$.
Otherwise $G_i$ is triangle-free, and any edge $Q_{i+1}$ is weakly-dominating
there.
