...
For any $2K_2$-free graph $G$, its vertex set $V(G)$ can be partitioned in two two parts, $V(F)$ and $V(D)$, where the induced subgraph $F$ admits a 2-walk having at most one vertex visited twice by the walk, and all other vertices visited exactly once by the walk, and induced subgraph $D$ is a coclique.
...
The following properties of $2K_2$-free graphs are very useful in the proof of Theorem \ref{thm2}.
...
Assume that $G$ is $2K_2$-free, with no isolated vertex, and $\omega(G)=2$, then $G$ is in one of the following cases.
\begin{enumerate}
...
Now, let us look at Theorem \ref{addgen1}.
In fact, we can prove a more detailed decomposition:
...
\item $Q_0$ is dominating in $G$ and Hamiltonian, 
\item $D$ is a coclique,
\item $Q_{\infty}$ admits a spanning 2-walk with at most one vertex visited more than once, 
\item $Q_{\infty}$ admits a spanning path (a.k.a. Hamiltonian path).
\end{enumerate}
\end{theorem}

First, we look at a special case of Theorem \ref{addgen1}, under the assumption $\omega(G)=2$, from which we can deduce the statement (\ref{thm1}.4) in Theorem \ref{thm1}. That is:
...
We denote
$S=\{v_{j,1}v_{j,2}\mid 1\leq j\leq \sigma\}$ as a maximun matching in $F$, and $E'_i=E_i\cap S$ (recall that $v_{j,1}v_{j,2}$
comprise $S_{j}$ as the bipartite case as well). 
Obviously,
...
\subsection{The Proof of Theorem \ref{addgen1}}
By Theorem \ref{thm4}, we only need to consider the case where $\omega(G)\ge3$.


Recall the sequence of cliques $\{Q_i\}$, $m$ and $s$ are as defined in Lemma \ref{lm5}.

If $s=0$ we have $Q=Q_0$, since $Q_{\infty}=\emptyset$. Then by Theorem \ref{thm1}, $Q$ is Hamiltonian.
Now let $s\geq 1$. The statement (\ref{thm1}.1) in Theorem \ref{thm1} provides 
...
Joining it to $u_1\cdots u_{2s}$, we obtain a Hamiltonian cycle in $Q$.
 
It remains to consider {$m=1$}.
If $u_1$ and $u_{2s}$ have two different neighbours in $Q_1$, say $u_1v_1, u_{2s}v_2\in E(G)$, where 
$v_1, v_2\in Q_1$, then we can pick up a spanning path in $Q_1$ with $v_1$ and $v_2$ as two endpoints. 
Connecting the spanning path in $Q_{\infty}$ with the spanning path in $Q_1$, we get a Hamiltonian cycle in $Q$.

If $u_1$ and $u_2$ are adjacent to only one vertex, say $v_{\infty}$ in $Q_1$, 
then we can find a Hamiltonian cycle in $Q_1$ such that the two endpoints of the spanning path 
in $Q_{\infty}$ are connected to $v_{\infty}$. This gives a 2-walk in $Q$, with $v_{\infty}$ visited twice and all other vertices visited once. 
\qed
...
Now, combine Lemma \ref{addtec} and Theorem \ref{addgen1}, we get Theorem \ref{thm2}.


Finally, let us state two conjectures.
\begin{conjecture}
Each $3/2$-tough $2K_2$-free graph with at least 3 vertices has a 2-trail, i.e. a 2-walk with each edge appearing in the walk at most once.
\end{conjecture}


\begin{conjecture}
Every 2-tough $2K_2$-free graph with at least vertices is Hamiltonian.
\end{conjecture}
