...
By Lemma \ref{addtec}, we only need to prove that the induced subgraph $Q$ on the vertex set $V(Q)=V(Q_0)\cup V(Q_{\infty})$ is Hamiltonian for $m\ge2$ 
(respectively, admits a 2-walk, with only one vertex $Q_{\infty}$ visited at most twice, and all other vertices visited exactly once for $m=1$.)

Let $G$ be a $(\frac{1}{k-1}+\epsilon)$-tough $2K_2$-free graph, for some $\epsilon>0$. The sequence of cliques $\{Q_i\}$, $m$ and $s$ are as defined in Lemma \ref{lm5}.

If $s=0$ we have $Q=Q_0$, since $Q_{\infty}=\emptyset$. Then by Theorem \ref{thm1}, $Q$ is Hamiltonian.
Now let $s\geq 1$. Corollary~\ref{cor:2w} provides 
