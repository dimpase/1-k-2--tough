...
clarity.  
%Let  $\ncc(G)$ denote the number of connected components of a graph $G$ with
%vertex set $V:=V(G)$ and edge set $E:=E(G)$. For $S\subset V$, let $G-S$ denote
%the subgraph of $G$ induced on $V-S$. A graph
%$G$ is called $\beta$-{\em tough}, for a real $\beta>0$, if $\ncc(G-S)>1$
%implies $|S|\ge \beta\cdot \ncc(G-S)$ for each $S\subset V$.
A {\em clique} means a complete subgraph, a {\em coclique} means a
induced subgraph containing no edge. 
%For $A=\{v_1,\ldots,v_m\}\subseteq V$, a
%subgraph $P$ in $G$ with $V(P)=A$ and
%$E(P)=\{v_1v_2,v_2v_3,\ldots,v_{m-1}v_m\}$ is called a {\em path}.  
For any
...
Then $V(H)$ admits a decomposition into two parts $V(F)$ and $V(\Delta)$. The induced subgraph $\Delta$ is a coclique, and the induced subgraph $F$ admitting 
...
If $G$ is $2K_2$-free with $\omega(G)\ge3$, then $G$ has a maximal (but not necessarily maximum) dominating clique $K$ of size at least 3, i.e. $|K|\ge3$. \qed
\end{theorem}

\begin{proof}
First, find a clique of size at least 3, i.e. a triangle say $x_1x_2x_3$. Then enlarge it to a maximal clique in $G$, by absorbing vertices adjacent to all vertices in the old clique. Denote such a maximal clique by $K=\{x_1,x_2,\ldots,x_p\}$, and $t=|V(G)-K^{\perp}|$. If $t=0$, then $K$ is dominating in $G$, so we may assume $t>0$. Let $Z=V(G)-K^{\perp}$. Because $p\ge3$, we have $Z$ is a coclique. For each $i=1,2,\ldots,p$, let $Y_i=\{y\in K^{\perp}|~yx_j\in E(G)~\text{if and only if}~j=i\}$. Since $p\ge3$, each $Y_i$ is a coclique.

Choose an arbitrary vertex $z_0\in Z$ and let $y_0\in K^{\perp}-K$ be any neighbor of $z_0$. Since $G$ is $2K_2$-free and $p$ is maximal, there is a unique integer $i\le p$ so that $y_0x_j\in E(G)$ if and only if $i\neq j$. Therefore $K'=(K-\{x_i\})\cup\{y_0\}$ is a clique of size $p$. Furthermore, any vertex dominated by $K$ is dominated by $K'$ except possibly those vertices in the set $Y'_i=\{y\in Y_i|~y_0y\not\in E(G)\}$.

Now, let us look at this set.
\begin{enumerate}
\item if $Y'_i=\emptyset$, then $(K')^{\perp}$ is strictly larger than $K^{\perp}$. We can apply to $K'$ all the things we have done on $K$ and rename $K'$ by $K$, until impossible.
\item if $Y'_i\neq\emptyset$, let $y_1\in Y'_i$. Then the edges $z_0y_0$ and $x_iy_1$ force $z_0y_1\in E(G)$. Choose distinct $j,k\in\{1,2,\ldots,p\}-\{i\}$. Then the edges $z_0y_1$ and $x_jx_k$ are independent edges. Contradiction.
\end{enumerate}
The contradiction shows that in the final step, $Z=V(G)-K^{\perp}=\emptyset$.
\end{proof}

\begin{remark}
In fact, Theorem \ref{lm2} is quite similar to \cite[Theorem 3]{chung1990maximum}, and our proof is almost the same as it, we include this proof only for sake of self-containing.
\qed
\end{remark}
